    ...
    <div className="group flex flex-row items-center w-full min-h-[72px] py-3 bg-[#222222] border border-[#666666]/20 hover:bg-white/[0.02] transition-colors relative">
      <div className="w-[240px] px-4 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-white/40 text-[11px] font-light truncate leading-none font-inter">{data.reportedBy.email}</span>
          </div>
        </div>
      </div>
      <div className="w-[200px] px-4 shrink-0 text-white text-[14px] font-medium truncate font-inter">{data.subject}</div>
      <div className="flex-[1] px-4 min-w-0 text-white/60 text-[14px] truncate font-inter">{data.message}</div>
      <div className="w-[120px] px-4 flex items-center shrink-0">
        <div className={`h-[32px] px-4 rounded-full text-[13px] font-bold flex items-center justify-center min-w-[84px] leading-none text-center ${getStatusColor(data.status)}`}>
          <span className="mt-[1px] font-inter">{data.status}</span>
        </div>
      </div>
      <div className="w-[200px] px-4 flex items-center gap-2 shrink-0">
        <span className="text-white text-[14px] leading-none font-inter">{data.submittedOn.split(' • ')[0]}</span>
        <div className="w-[3px] h-[3px] bg-white rounded-full opacity-40 shrink-0" />
        <span className="text-white text-[14px] opacity-40 leading-none font-inter">{data.submittedOn.split(' • ')[1]}</span>
      </div>
          ...
          <div ref={menuRef} className="absolute top-[80%] right-full mr-2 w-[240px] bg-[#1a1a1a] border border-[#666666]/50 rounded-[16px] shadow-2xl z-[60] overflow-hidden py-2 animate-in fade-in slide-in-from-right-2 duration-200">
            {[
              ...
              >
                ...
                <span className="flex-grow text-white text-[14px] font-inter">{item.label}</span>
