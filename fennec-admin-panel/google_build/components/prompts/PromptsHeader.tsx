  ...
  return (
    <div className="flex flex-row items-end justify-between w-full max-w-[1520px] h-[80px] mb-10 animate-in fade-in slide-in-from-top-4 duration-700">
      ...
      <div className="flex flex-row items-center gap-4 h-[56px]">
        ...
        <button 
          onClick={onAddPrompt}
          className="flex flex-row items-center justify-center px-6 py-4 gap-3 h-full bg-[#5F00DB] shadow-[0px_4px_12px_rgba(95,0,219,0.25)] rounded-[52px] transition-all hover:brightness-110 active:scale-95"
        >
          <span className="text-white text-[16px] font-medium leading-[24px]">{addLabel}</span>
        </button>
      </div>
    </div>
  );
