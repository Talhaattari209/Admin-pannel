...
import { useSuperAdminLogin, useForgotPassword } from '@/services/auth';

const LoginPage = () => {
    const [currentView, setCurrentView] = useState<'login' | 'forgot' | 'sent'>('login');
    const [errorMessage, setErrorMessage] = useState<string>('');
    const [forgotErrorMessage, setForgotErrorMessage] = useState<string>('');
    ...
    const handleLogin = async (data: any) => {
        ...
        try {
            ...
            const message = error?.response?.data?.detail || error?.response?.data?.message || error.message || 'Invalid credentials. Please try again.';
    ...
    const handleSendLink = async (email: string) => {
        ...
        try {
            ...
            const message = error?.response?.data?.message || error?.response?.data?.detail || error.message || 'Something went wrong. Please try again.';
            setForgotErrorMessage(message);
        }
    };
        ...
        <div className="relative w-full h-screen overflow-hidden flex items-center justify-center bg-[#050511]">
            ...
            <div className="relative z-10 w-full flex justify-center px-[2vw]">
                {currentView === 'login' && (
                    <LoginForm
                        onLogin={handleLogin}
                        onForgotPassword={() => { setForgotErrorMessage(''); setCurrentView('forgot'); }}
                    />
                )}
                ...
                {currentView === 'forgot' && (
                    <ForgotPasswordCard
                        ...
                        onBack={() => { setForgotErrorMessage(''); setCurrentView('login'); }}
