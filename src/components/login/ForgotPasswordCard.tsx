...
const ForgotPasswordCard: React.FC<ForgotPasswordCardProps> = ({ onSendLink, onBack, isLoading = false, errorMessage }) => {
          ...
          <div className="flex flex-col items-start gap-[0.2vw] w-full">
            ...
            <div className="flex flex-col w-full border-b border-white py-[0.4vw]">
              <input
                ...
                disabled={isLoading}
                className="w-full bg-transparent border-none text-white text-[0.83vw] focus:outline-none placeholder:text-white/40 disabled:opacity-50"
                placeholder="Enter your email"
              />
            </div>
            {/* Inline error */}
            {errorMessage && (
              <p className="text-red-400 text-[0.72vw] mt-[0.2vw]">{errorMessage}</p>
            )}
          </div>
        </div>
          ...
          >
            {isLoading ? (
              <span className="flex items-center gap-[0.5vw]">
                ...
                <span className="font-medium not-italic text-[0.83vw] leading-[1.5] text-white text-center">Sending...</span>
              </span>
            ) : (
