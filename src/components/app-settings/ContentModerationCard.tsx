...
const ContentModerationCard: React.FC<ContentModerationCardProps> = ({ canEdit = true }) => {
  ...
  const Switch = ({ active, onClick }: { active: boolean, onClick: () => void }) => (
    <button
      onClick={onClick}
      disabled={!canEdit}
      className={`w-[2.5vw] h-[1.46vw] rounded-full border border-white transition-all duration-300 relative ${active ? 'bg-transparent' : 'bg-transparent opacity-30'} ${!canEdit ? 'opacity-50 cursor-not-allowed' : ''}`}
    >
