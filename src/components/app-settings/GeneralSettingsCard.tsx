...
const GeneralSettingsCard: React.FC = () => {
  ...
  const Field = ({ label, value, id }: { label: string, value: string, id: keyof typeof settings }) => (
    <div className="flex flex-col gap-[0.42vw] border-b border-white py-[0.21vw]">
      <label className="text-white text-[0.63vw] font-bold uppercase tracking-widest opacity-80 font-inter">{label}</label>
      <input
        type="number"
        value={value}
  ...
  return (
    <div className="flex flex-col p-[1.25vw] bg-[#222222] border border-[#666666]/30 rounded-[0.83vw] gap-[1.67vw] shrink-0 shadow-lg min-h-[21.77vw]">
      <div className="flex flex-col gap-[0.42vw]">
        <h3 className="text-white text-[1.46vw] font-bold font-inter leading-none tracking-tight">General Settings</h3>
        <p className="text-[#CCCCCC] text-[0.83vw] opacity-60 leading-relaxed font-inter">Adjust the main rules and limits that define Fennec’s experience — from group sizes to interaction limits.</p>
      </div>
