        ...
        <div className={`flex flex-row gap-[0.42vw] w-full overflow-x-auto no-scrollbar shrink-0 ${className}`}>
            {tabs.map((tab) => (
                ...
                >
                    <span>{tab.label}</span>
                    {tab.count !== undefined && (
                        <div className={`flex items-center justify-center min-w-[0.94vw] h-[0.94vw] px-[0.42vw] rounded-full ml-[0.42vw] bg-[#FF4E4E] text-white text-[0.57vw] font-bold not-italic leading-none pb-[0.05vw]`}>
                            {tab.count}
                        </div>
                    )}
                </button>
