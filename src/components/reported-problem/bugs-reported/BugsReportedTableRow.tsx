...
const BugsReportedTableRow: React.FC<BugsReportedTableRowProps> = ({ data, onAction }) => {
    ...
    return (
        <div className="group flex flex-row items-center w-full min-h-[72px] py-3 bg-[#222222] border border-[#666666]/20 hover:bg-white/[0.02] transition-colors relative">
            <div className="w-[16.66vw] max-w-[240px] px-4 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col min-w-0">
                        <span className="text-white text-[14px] font-semibold truncate leading-none mb-1 font-['SF_Pro_Text']">{data.reportedBy.name}</span>
                        <span className="text-white/40 text-[11px] font-light truncate leading-none font-['SF_Pro_Text']">{data.reportedBy.email}</span>
                    </div>
                </div>
            </div>
            <div className="w-[13.88vw] max-w-[200px] px-4 shrink-0 text-white text-[14px] font-medium truncate font-['SF_Pro_Text']">{data.subject}</div>
            <div className="flex-[1] px-4 min-w-0 text-white/60 text-[14px] truncate font-['SF_Pro_Text']">{data.message}</div>
