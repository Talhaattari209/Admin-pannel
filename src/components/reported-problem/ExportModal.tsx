            ...
            <div className="relative flex flex-col items-center p-[2.5vw] gap-[2.5vw] bg-[#16003F] border border-[#666666]/50 backdrop-blur-[12px] rounded-[2.5vw] box-border shadow-2xl overflow-hidden w-[25vw] min-w-[320px]">
                ...
                <div className="flex flex-col justify-center items-center p-[0.83vw] gap-[0.83vw] isolation-auto w-[6.25vw] h-[6.25vw] rounded-[1.25vw] flex-none order-0 flex-grow-0 relative">
                    ...
                    <div className="relative z-10 flex items-center justify-center">
                        <svg className="w-[2.92vw] h-[2.92vw] text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                        ...
                        <div className="flex gap-[0.83vw]">
                            {['CSV', 'PDF', 'Excel'].map((f) => (
                                <button
                                    key={f}
                                    onClick={() => setFormat(f)}
                                    className={`flex-1 h-[2.5vw] rounded-[0.63vw] border flex items-center justify-center text-[0.73vw] font-medium not-italic transition-all cursor-pointer ${format === f
                ...
                <div className="flex flex-row items-center justify-center gap-[1.25vw] self-stretch mt-auto">
                    <button
                        onClick={onClose}
                        className="flex-1 h-[3.5vw] border border-white/30 bg-transparent rounded-[2.71vw] text-white font-medium not-italic text-[0.94vw] hover:bg-white/5 transition-all cursor-pointer"
                    >
                        Cancel
                    </button>
                    <button
                        onClick={() => onExport(format, dateRange)}
                        className="flex-1 h-[3.5vw] bg-[#5F00DB] rounded-[2.71vw] text-white font-medium not-italic text-[0.94vw] shadow-lg hover:brightness-110 active:scale-95 transition-all cursor-pointer"
                    >
                        Export Data
                    </button>
                </div>
