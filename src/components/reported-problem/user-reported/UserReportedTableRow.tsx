...
const UserReportedTableRow: React.FC<UserReportedTableRowProps> = ({ data, onAction }) => {
    ...
    const UserCell = ({ user }: { user: { name: string; email: string; avatar: string } }) => (
        <div className="flex items-center gap-[0.42vw] min-w-0">
            ...
            <div className="flex flex-col min-w-0 gap-[0.21vw]">
                <span className="text-white text-[0.73vw] font-normal not-italic leading-[114%] truncate font-['SF_Pro_Text']">{user.name}</span>
                <span className="text-white/50 text-[0.57vw] font-light not-italic leading-[145%] tracking-[0.02em] truncate font-['SF_Pro_Text']">{user.email}</span>
            </div>
        </div>
    );
    ...
    return (
        <div className="group flex flex-row items-center w-full h-[2.92vw] bg-[#222222] border-b border-[rgba(102,102,102,0.5)] hover:bg-white/[0.05] transition-colors relative shrink-0">
            ...
            <div className="w-[9.38vw] px-[0.63vw] shrink-0 text-white text-[0.73vw] leading-[114%] font-['SF_Pro_Text'] not-italic flex items-center">{data.category}</div>
            <div className="w-[6.25vw] px-[0.63vw] shrink-0 text-white text-[0.73vw] leading-[114%] font-['SF_Pro_Text'] not-italic flex items-center">{data.reports}</div>
            <div className="flex-[1] px-[0.63vw] min-w-0 text-white/60 text-[0.73vw] leading-[114%] truncate font-['SF_Pro_Text'] not-italic flex items-center">{data.description}</div>
            <div className="w-[6.25vw] px-[0.63vw] flex items-center shrink-0">
                <div className={`h-[1.67vw] px-[0.63vw] rounded-[0.83vw] text-[0.73vw] font-normal not-italic flex items-center justify-center min-w-[2.81vw] leading-[114%] text-center font-['SF_Pro_Text'] ${getStatusColor(data.status)}`}>
                    <span className="mt-[0.1vw]">{data.status}</span>
                </div>
            </div>
            <div className="w-[10.42vw] px-[0.63vw] flex items-center gap-[0.42vw] shrink-0 font-['SF_Pro_Text'] not-italic">
            ...
            <div className="w-[2.5vw] px-[0.42vw] flex justify-center items-center relative shrink-0">
                <button
                    ...
                    className={`flex flex-row justify-center items-center p-[0.42vw] gap-[0.83vw] w-[1.67vw] h-[1.67vw] rounded-[2.71vw] transition-colors ${isMenuOpen ? 'bg-white/10 text-white' : 'hover:bg-white/10 text-white group-hover:text-white'}`}
                >
