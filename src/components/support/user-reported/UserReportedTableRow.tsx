...
interface UserReportedTableRowProps {
  data: UserReportData;
  onAction: (action: 'DETAILS' | 'STATUS' | 'PROFILE' | 'REPORTED_PROFILE', data: UserReportData) => void;
}
...
const UserReportedTableRow: React.FC<UserReportedTableRowProps> = ({ data, onAction }) => {
  ...
  const UserCell = ({ user }: { user: { name: string; email: string; avatar: string } }) => (
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="flex flex-col min-w-0">
        <span className="text-white text-[14px] font-semibold not-italic truncate leading-none mb-1">{user.name}</span>
        <span className="text-white/40 text-[11px] font-light not-italic truncate leading-none">{user.email}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="group flex flex-row items-center w-full min-h-[72px] py-3 bg-[#222222] border border-[#666666]/20 hover:bg-white/[0.02] transition-colors relative">
      <div className="w-[240px] px-4 shrink-0"><UserCell user={data.reportedBy} /></div>
      <div className="w-[240px] px-4 shrink-0"><UserCell user={data.reportedUser} /></div>
      <div className="w-[180px] px-4 shrink-0 text-white text-[14px] not-italic">{data.category}</div>
      <div className="w-[120px] px-4 shrink-0 text-white text-[14px] not-italic">{data.reports}</div>
      <div className="flex-[1] px-4 min-w-0 text-white/60 text-[14px] not-italic truncate">{data.description}</div>
      ...
      <div className="w-[200px] px-4 flex items-center gap-2 shrink-0">
        <span className="text-white text-[14px] not-italic leading-none">{data.submittedOn.split(' • ')[0]}</span>
        <div className="w-[3px] h-[3px] bg-white rounded-full opacity-40 shrink-0" />
        <span className="text-white text-[14px] not-italic opacity-40 leading-none">{data.submittedOn.split(' • ')[1]}</span>
      </div>
      ...
      <div className="w-[60px] px-2 flex justify-center relative shrink-0">
        ...
        {isMenuOpen && (
          <div
            ref={menuRef}
            className="absolute top-[80%] right-full mr-2 w-[280px] bg-[#1a1a1a] border border-[#666666]/50 rounded-[16px] shadow-2xl z-[60] overflow-hidden py-2 animate-in fade-in slide-in-from-right-2 duration-200"
          >
            {[
              ...
              { id: 'PROFILE', label: "View Reporter's Profile", icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg> },
              { id: 'REPORTED_PROFILE', label: "View Reported User's Profile", icon: <svg className="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg> }
            ].map((item) => (
              <button
                key={item.id}
