...
const UserReportedTableRow: React.FC<UserReportedTableRowProps> = ({ data, onAction }) => {
  ...
  const UserCell = ({ user }: { user: { name: string; email: string; avatar: string } }) => (
    <div className="flex items-center gap-3 min-w-0">
      ...
      <div className="flex flex-col min-w-0">
        <span className="text-white text-[14px] font-semibold truncate leading-none mb-1">{user.name}</span>
        <span className="text-white/40 text-[11px] font-light truncate leading-none">{user.email}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="group flex flex-row items-center w-full min-h-[72px] py-3 bg-[#222222] border border-[#666666]/20 hover:bg-white/[0.02] transition-colors relative">
      ...
      <div className="w-[180px] px-4 shrink-0 text-white text-[14px]">{data.category}</div>
      <div className="w-[120px] px-4 shrink-0 text-white text-[14px]">{data.reports}</div>
      <div className="flex-[1] px-4 min-w-0 text-white/60 text-[14px] truncate">{data.description}</div>
