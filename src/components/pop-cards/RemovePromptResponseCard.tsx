            ...
            <div className="relative flex flex-col items-center p-[1.66vw] gap-[1.66vw] w-[25vw] max-h-[88vh] bg-[#16003F] border border-[#666666]/50 backdrop-blur-[12px] rounded-[1.66vw] box-border shadow-2xl overflow-y-auto scrollbar-hide">
                ...
                <div className="flex flex-col gap-[0.83vw] text-center self-stretch">
                    ...
                    <p className="text-[0.93vw] font-normal not-italic leading-[1.66vw] opacity-90 text-white">
                        The selected prompt response will be removed. Choose a reason to notify the user.
                    </p>
                </div>
                ...
                <div className="flex flex-col self-stretch gap-[1.66vw] w-full">
                    <div className="flex flex-col self-stretch bg-[#111111]/25 border border-[#444444]/50 rounded-[0.83vw] overflow-hidden">
                        {REASONS.map((reason) => (
                            <button key={reason} onClick={() => setSelectedReason(reason)} className="flex items-center p-[0.83vw] gap-[0.83vw] border-b border-[#444444]/50 last:border-b-0 hover:bg-white/5 transition-colors cursor-pointer">
                ...
                <div className="flex flex-row items-center justify-center gap-[1.25vw] self-stretch mt-auto">
                    <button onClick={onCancel} className="flex-1 h-[3.5vw] border border-white/20 bg-transparent rounded-[2.71vw] text-white font-medium not-italic text-[0.94vw] hover:bg-white/10 filter drop-shadow(0px 0.63vw 2.08vw rgba(0, 0, 0, 0.05)) backdrop-blur-[6px] transition-all cursor-pointer">Cancel</button>
                    <button onClick={() => onRemove(selectedReason, customReason)} className="flex-1 h-[3.5vw] bg-[#5F00DB] rounded-[2.71vw] text-white font-medium not-italic text-[0.94vw] shadow-[0px_-0.42vw_0.63vw_rgba(95,0,219,0.25),0px_0.42vw_0.63vw_rgba(95,0,219,0.25)] hover:bg-[#7000ff] transition-all cursor-pointer">Remove</button>
                </div>
            </div>
        </PopCardWrapper>
    );
