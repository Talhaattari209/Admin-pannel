...
const BugsReportedCard: React.FC = () => {
  ...
  return (
    <div className="flex flex-col items-start bg-[#222222] rounded-[16px] w-full h-[580px] shadow-2xl overflow-hidden border border-white/5">
      <div className="flex flex-col items-start p-6 gap-2 w-full shrink-0 h-[117px]">
        <h4 className="text-white text-[24px] font-bold leading-[120%] tracking-[-0.04em] font-inter">
          Bugs Reported
        </h4>
      ...
      <div className="relative flex flex-row w-full h-[463px] px-2 py-6">
        <div className="relative flex-grow h-full flex items-center justify-center">
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none z-10">
            <span className="text-white text-[32px] font-bold font-inter">{total}</span>
          </div>
        ...
        <div className="w-[120px] flex flex-col justify-center gap-4 pr-6 shrink-0">
          {data.map((item, index) => (
            <div key={index} className="flex items-center gap-2">
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }} />
              <span className="text-white/80 text-[12px] font-inter whitespace-nowrap">{item.name}</span>
            </div>
          ))}
        </div>
      </div>
