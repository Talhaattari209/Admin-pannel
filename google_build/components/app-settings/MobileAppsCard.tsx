...
const MobileAppsCard: React.FC = () => {
  const AppTile = ({ platform, version, isAndroid }: { platform: string, version: string, isAndroid: boolean }) => (
    <div className="flex-1 flex flex-row items-center p-4 bg-[#16003F] border border-white/5 rounded-[16px] gap-4 group">
      <div className="w-[72px] h-[72px] bg-[#5F00DB] rounded-[16px] flex items-center justify-center shrink-0 shadow-xl overflow-hidden relative">
        <div className="absolute inset-0 bg-white/5 group-hover:bg-white/10 transition-colors"></div>
        <svg viewBox="0 0 100 100" className="w-[50px] h-[50px] relative z-10">
      ...
      <div className="flex flex-col flex-grow min-w-0">
        <span className="text-white text-[20px] font-bold font-inter leading-none mb-1">{platform}</span>
      ...
      <div className="flex flex-row items-center gap-4">
        ...
        <button className="flex items-center gap-2 px-4 py-2 bg-[#5F00DB] rounded-[52px] text-white text-[14px] font-medium hover:brightness-110 shadow-lg whitespace-nowrap">
          {isAndroid ? 'Play Store' : 'App Store'}
          <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6M15 3h6v6M10 14L21 3"/></svg>
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col p-6 bg-[#222222] border border-[#666666]/30 rounded-[16px] gap-8 shrink-0 shadow-lg">
      <div className="flex flex-col gap-2">
        <h3 className="text-white text-[28px] font-bold font-inter leading-none tracking-tight">Mobile Apps</h3>
        <p className="text-[#CCCCCC] text-[16px] opacity-60">Latest versions of your iOS and Android Mobile Apps</p>
      </div>
