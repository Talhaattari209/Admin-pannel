...
const BugReportDetailsView: React.FC<BugReportDetailsViewProps> = ({ bug, onBack }) => {
  ...
  return (
    <div className="flex flex-col w-full max-w-[1520px] mx-auto p-4 md:p-8 h-screen animate-in fade-in duration-500 overflow-hidden">
      
      {/* Window Header - No Buttons on the right as per design */}
      <div className="flex flex-row items-center gap-4 w-full h-[80px] mb-10 shrink-0">
        ...
        <div className="flex flex-col flex-grow">
          <h1 className="text-white text-[36px] font-bold tracking-tight font-inter">Bug Report Details</h1>
      ...
      <div className="flex flex-row gap-6 w-full items-start h-full overflow-hidden pb-10">
        
        {/* Left Column: Details & Activity (flex-[3]) - scrollable */}
        <div className="flex flex-col gap-6 flex-[3] min-w-0 h-full overflow-y-auto no-scrollbar pb-10">
          
          {/* Reported Bug Card - With Vertical Invisible Scroll */}
          <div className="flex flex-col p-8 bg-[#222222] border border-[#666666]/30 rounded-[24px] shrink-0 shadow-lg">
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-white text-[28px] font-bold leading-tight font-inter">Reported Bug</h3>
            ...
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-white text-[18px] font-normal font-inter">{bug.subject}</p>
              </div>
              
              <div className="flex flex-col gap-2">
                <label className="text-white text-[12px] font-bold uppercase tracking-wider opacity-60">Message</label>
                <p className="text-white text-[18px] font-normal font-inter leading-relaxed opacity-90">{bug.message}</p>
              </div>
                  ...
                  {(bug.attachments || ['https://images.unsplash.com/photo-1550439062-609e1531270e?auto=format&fit=crop&q=80&w=800']).map((img, i) => (
                    ...
                    >
                      ...
                      <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 ${revealedImages[i] ? 'bg-transparent' : 'bg-black/40'}`}>
                         ...
                         {!revealedImages[i] && <span className="text-white text-[12px] font-bold mt-4 tracking-widest uppercase opacity-80 select-none font-inter">Click to reveal</span>}
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* Activity Card */}
          <div className="flex flex-col p-8 bg-[#222222] border border-[#666666]/30 rounded-[24px] shrink-0 shadow-xl">
            ...
            <div className="flex flex-col relative pl-8 gap-12 pb-10">
              ...
              <div className="relative flex flex-col items-start gap-4">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <span className="text-white/60 text-[16px] font-inter">{bug.submittedOn}</span>
                </div>
                ...
                <div className="flex flex-col p-5 bg-[#16003F] border border-white/5 rounded-[16px] w-[280px] gap-4 mt-2 group shadow-2xl transition-all hover:border-[#5F00DB]/40 cursor-default">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col flex-grow min-w-0">
                      <span className="text-white text-[14px] font-semibold truncate leading-none mb-1 font-inter">{bug.reportedBy.name}</span>
                      <span className="text-white/40 text-[11px] truncate leading-none font-light font-inter">{bug.reportedBy.email}</span>
                    </div>
                    <svg viewBox="0 0 24 24" className="w-6 h-6 text-white opacity-40 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" strokeWidth="2.5">
        ...
        {/* Right Column: Actions Sidebar - Sticky (flex-[1]) */}
        <div className="flex-[1] flex flex-col gap-6 bg-[#222222] border border-[#666666]/30 rounded-[24px] p-8 sticky top-0 min-w-[340px] shadow-2xl">
