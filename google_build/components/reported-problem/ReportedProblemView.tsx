...
const ReportedProblemView: React.FC<ReportedProblemViewProps> = ({ 
  ...
  return (
    <div className="flex flex-col items-center w-full px-[2vw] py-[1vw] animate-in fade-in duration-700">
      <div className="flex flex-row items-end justify-between w-full max-w-[95vw] h-[80px] mb-10 shrink-0">
        <div className="flex flex-col justify-center items-start gap-2 max-w-[70vw]">
          ...
          <p className="text-[#CCCCCC] text-[1vw] leading-[150%] opacity-60">Review and resolve user reports on inappropriate content, messages, or behavior.</p>
        </div>
        <button 
          onClick={onExport}
      ...
      <div className="flex flex-row gap-[1.5vw] w-full max-w-[95vw] mb-10 overflow-hidden shrink-0">
        {stats.map((stat, i) => (
          <div key={i} className="flex flex-col justify-between p-[1.5vw] flex-1 h-[123px] bg-[#16003F] border border-[#666666]/30 rounded-[1.5vw] relative overflow-hidden group shadow-lg">
            ...
            <span className="text-white/80 text-[0.8vw] font-bold uppercase tracking-wider z-10">{stat.label}</span>
            <span className="text-white text-[2.5vw] font-bold tracking-tight leading-none z-10">{stat.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-row gap-2 pl-4 w-full max-w-[95vw] mb-[-1px] z-10 shrink-0">
        {[
          ...
          <button
            ...
            className={`flex flex-row items-center justify-center px-[2vw] py-[0.8vw] gap-2 h-[48px] rounded-t-[12px] transition-all font-medium text-[0.9vw] ${
              activeTab === tab.id ? 'bg-[#5F00DB] text-white' : 'bg-[#222222] text-white/60 hover:text-white hover:bg-white/5'
            }`}
          >
            {tab.label}
