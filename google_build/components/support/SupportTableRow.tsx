    ...
    <div className="group flex flex-row items-center w-full min-h-[72px] py-3 bg-[#222222] border border-[#666666]/20 hover:bg-white/[0.02] transition-colors relative">
      ...
      <div className="w-[200px] px-4 flex items-center gap-3 shrink-0">
        ...
        <div className="flex flex-col min-w-0">
          <span className="text-white text-[14px] font-semibold truncate leading-none mb-1">{data.user.name}</span>
          <span className="text-white/40 text-[11px] font-light truncate leading-none">{data.user.email}</span>
        </div>
      </div>

      {/* Subject Cell */}
      <div className="flex-[1] px-4 min-w-0">
        <p className="text-white text-[14px] font-medium truncate">{data.subject}</p>
      </div>

      {/* Message Cell */}
      <div className="flex-[1] px-4 min-w-0">
        <p className="text-white/60 text-[14px] font-normal truncate">{data.message}</p>
      </div>

      {/* Status Cell */}
      <div className="w-[200px] px-4 flex items-center shrink-0">
      ...
      <div className="w-[200px] px-4 flex items-center gap-2 shrink-0">
        <span className="text-white text-[14px] leading-none">{data.timestamp.split(' • ')[0]}</span>
      ...
      <div className="w-[48px] px-2 flex justify-center relative shrink-0">
        ...
        {isMenuOpen && (
          <div
            ref={menuRef}
            className="absolute top-[80%] right-full mr-2 w-[240px] bg-[#1a1a1a] border border-[#666666]/50 rounded-[16px] shadow-2xl z-[60] overflow-hidden py-2 animate-in fade-in slide-in-from-right-2 duration-200"
          >
            {[
              {
              ...
              >
                ...
                <svg viewBox="0 0 24 24" className="w-4 h-4 text-white opacity-40" fill="none" stroke="currentColor" strokeWidth="2">
