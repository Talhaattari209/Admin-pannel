    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="relative flex flex-col items-center p-8 gap-8 w-[480px] bg-[#16003F] border border-[#666666]/50 rounded-[32px] shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <div className="flex flex-col gap-4 text-center">
          <h2 className="text-white text-[32px] font-medium tracking-tight font-inter">{isMember ? 'Remove Team Member?' : 'Delete Role?'}</h2>
          <p className="text-white/80 text-[16px] leading-relaxed font-inter px-4">
            {isMember 
        ...
        <div className="flex flex-row gap-6 w-full">
          <button 
            onClick={onCancel}
            className="flex-1 h-[56px] border border-white rounded-[52px] text-white font-medium hover:bg-white/10 transition-all font-inter"
          >
            Cancel
          </button>
          <button 
            onClick={onConfirm}
            className="flex-1 h-[56px] bg-[#FF4E4E] rounded-[52px] text-white font-medium shadow-[0px_4px_12px_rgba(255,78,78,0.3)] hover:brightness-110 active:scale-95 transition-all font-inter"
          >
